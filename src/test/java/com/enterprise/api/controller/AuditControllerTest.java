...
class AuditControllerTest {
    ...
    void getAuditLogsByUser_WithDateRange_ShouldReturnFilteredAuditLogs() throws Exception {
        ...
        when(auditService.findByPerformedByAndDateRange(eq("testuser"), eq(startDate), eq(endDate), any(Pageable.class)))
                .thenReturn(sampleAuditPage);
    ...
    void getAuditStatistics_ShouldReturnStatistics() throws Exception {
        ...
        when(auditService.getAuditStatsByOperation()).thenReturn(operationCounts);
        when(auditService.findByDateRange(any(LocalDateTime.class), any(LocalDateTime.class), any(Pageable.class)))
    ...
    void getAuditStatistics_WithDateRange_ShouldReturnFilteredStatistics() throws Exception {
        ...
        when(auditService.getAuditStatsByOperation()).thenReturn(Map.of("CREATE", 5L));
        when(auditService.findByDateRange(eq(startDate), eq(endDate), any(Pageable.class)))
    ...
    @Test
    @WithMockUser(roles = "ADMIN")
    void getAllAuditLogs_WithInvalidSortDirection_ShouldUseDefault() throws Exception {
        when(auditService.findAll(any(Pageable.class))).thenReturn(sampleAuditPage);

        mockMvc.perform(get("/api/v1/audit")
                .param("sortDir", "invalid"))
                .andExpect(status().isOk()); // Should use default sort direction
    }
}