...
/**
 * Security tests for authentication bypass attempts and security vulnerabilities.
 */
@SpringBootTest
@AutoConfigureWebMvc
@ActiveProfiles("test")
...
class SecurityBypassTest {
    ...
    void testPrivilegeEscalationAttempts() throws Exception {
        ...
        // Extract token (simplified - in real test you'd parse JSON)
        String token = "Bearer " + response.split("\"token\":\"")[1].split("\"")[0];
    ...
    void testSessionFixationAttempts() throws Exception {
        // Test with custom session ID in header
        mockMvc.perform(post("/api/auth/login")
        ...
        // Verify that the application doesn't use the provided session ID
        // (JWT-based auth should not be vulnerable to session fixation)
    }
    ...
    void testCsrfProtection() throws Exception {
        ...
        mockMvc.perform(post("/api/users")
                ...
                .andExpect(status().isUnauthorized()); // Should fail due to missing auth, not CSRF
    }
    ...
    void testXssPreventionInResponses() throws Exception {
        ...
        maliciousUser.setUsername("<script>alert('xss')</script>");
        maliciousUser.setEmail("xss@test.com");
        maliciousUser.setPassword(passwordEncoder.encode("Password123!"));
        userRepository.save(maliciousUser);

        // Verify that the response is properly escaped
        mockMvc.perform(get("/api/users")
                .header("Authorization", "Bearer valid_admin_token"))
                .andExpect(status().isUnauthorized()) // Will fail auth, but that's expected
    ...
    void testPathTraversalAttempts() throws Exception {
        ...
        for (String pathAttempt : pathTraversalAttempts) {
            mockMvc.perform(get("/api/files/" + pathAttempt))
                    .andExpect(status().isUnauthorized()); // Should fail auth first
        }
    }
    ...
    void testTimingAttackResistance() throws Exception {
        ...
        // Note: This is a basic test - real timing attack testing requires more sophisticated analysis
    }
    ...
    void testXmlExternalEntityAttacks() throws Exception {
        ...
        mockMvc.perform(post("/api/users")
                ...
                .content(xxePayload))
                .andExpect(status().isUnsupportedMediaType()); // Should reject XML
    }
    ...
    void testTemplateInjectionAttacks() throws Exception {
        ...
        for (String payload : templateInjectionPayloads) {
            mockMvc.perform(post("/api/users")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{\"username\":\"" + payload + "\",\"email\":\"test@test.com\",\"password\":\"Password123!\"}"))
                    .andExpect(status().isBadRequest());
        }
    }
    ...
    void testPrivilegeEscalationParameterPollution() throws Exception {
        ...
        mockMvc.perform(post("/api/users")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"username\":\"pollutiontest\",\"email\":\"pollution@test.com\",\"password\":\"Password123!\"}"))
    ...
    void testBusinessLogicBypassAttempts() throws Exception {
        ...
        mockMvc.perform(post("/api/users")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"username\":\"negativetest\",\"email\":\"negative@test.com\",\"password\":\"Password123!\",\"age\":-1}"))
                .andExpect(status().isBadRequest());

        // Test extremely large values
        mockMvc.perform(post("/api/users")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"username\":\"largetest\",\"email\":\"large@test.com\",\"password\":\"Password123!\",\"age\":999999}"))
                .andExpect(status().isBadRequest());
    }
}